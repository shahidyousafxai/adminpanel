...
const QuickLinks = () => {
    ...
    const handleEditItem = (id) => {
        states.setModalFor('Edit Quick Link')
        states.setEditLinks('footer')
        states.setEditId(id);
        states.setOpen(!states.open);
        //populate modal with data
        const item = states.quickLinks.find((item) => item.id === id);
    ...
    const handleDelete = (id) => {
        const newQuickLinks = states.quickLinks.filter((item) => item.id !== id);
        const updatedLinks = newQuickLinks.map((item, index) => {
            return {
                ...item,
                id: index + 1
            }
        })
        states.setQuickLinks(updatedLinks);
    }


    const quickLinks = states.quickLinks.map((item) => {
        ...
        <div className='flex items-center justify-center'>
            <div className="w-[90%] mt-8 mb-8">
                <div className="mb-4 flex items-center justify-between">
                    <h3 className="text-lg font-bold">Add Quick Link</h3>
                    <button className="px-4 py-2 flex justify-end rounded-md bg-p-light-navy text-white" onClick={() => handleModuleItemClick("Add New Quick Link")}>Add +</button>
                </div>
                        ...
                        <div className="inline-block min-w-full overflow-hidden align-middle border-b border-gray-200 sm:rounded-lg">
                            <table className="min-w-full border">
                                ...
                                <tbody className="bg-white">
                                    {React.Children.toArray(quickLinks)}
                                </tbody>
                            </table>
                        </div>
                    </div>
